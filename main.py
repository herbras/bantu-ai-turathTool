...
async def lifespan(app: FastAPI):
    ...
    async with MCPTools(transport="sse", url=mcp_server_url) as mcp_tools:
        ...
        from src.workflows.turath_research_workflow import set_global_agent_service

        set_global_agent_service(agent_service)
        print("✅ Agents Initialized and global service set.")

        initialized_agent_instances = agent_service.get_all_agents()

        v1_api_router = await create_v1_router(
            initialized_agents=initialized_agent_instances,
            agent_service=agent_service,  # Pass agent service to inject into workflows
        )
