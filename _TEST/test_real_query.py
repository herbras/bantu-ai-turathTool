...
async def test_real_query():
    ...
    async with MCPTools(transport="sse", url="http://localhost:8001/sse") as mcp_tools:
        print(
            f"✅ MCPTools connected with functions: {list(mcp_tools.functions.keys())}"
        )
        ...
        print(
            f"✅ Agent tools: {[tool.name if hasattr(tool, 'name') else tool.__class__.__name__ for tool in agent.tools]}"
        )
