...
async def get_turath_research_team() -> Team:
    ...
    try:
        logger.info("Initializing MCPTools with URL: %s using SSE transport...", mcp_server_url_val)
        mcp_tools = MCPTools(transport="sse", url=mcp_server_url_val) 
        logger.info("MCPTools Initialized.")

        logger.info("MCPTools instance ready for agent use.")

    except Exception as e:
        logger.warning("Could not initialize MCPTools in get_turath_research_team. Error: %s", e, exc_info=True)
        mcp_tools = None
    ...
    if hasattr(turath_query_member_agent, 'initialize') and callable(getattr(turath_query_member_agent, 'initialize')):
        logger.info("Initializing member agent: %s...", turath_query_member_agent.name)
        await turath_query_member_agent.initialize()
        logger.info("Member agent %s initialized.", turath_query_member_agent.name)
    else:
        logger.warning("Member agent %s does not have an initialize method.", turath_query_member_agent.name)
