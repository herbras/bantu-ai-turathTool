...
try:
    ...
    logger.warning("ArxivTools or PubmedTools not available. Install with: pip install arxiv pypdf")
    ArxivTools = None
    PubmedTools = None
...
class TurathScientificTools(Toolkit):
    ...
    def search_arxiv_with_islamic_context(self, query: str, max_results: int = 5) -> str:
    ...
    def search_pubmed_with_islamic_context(self, query: str, max_results: int = 5) -> str:
        ...
        try:
            ...
            formatted_results += f"## Islamic Bioethics Perspective\n{bioethics_note}\n\n"
    ...
    def search_scientific_literature(self, query: str, search_type: str = "auto") -> str:
        ...
        if search_type == "auto":
            medical_keywords = ['medical', 'health', 'disease', 'treatment', 'therapy', 'clinical', 
                              'patient', 'medicine', 'bioethics', 'pharmaceutical', 'genetics']
            tech_keywords = ['technology', 'ai', 'artificial intelligence', 'computer', 'algorithm',
        ...
        if search_type in ["medical", "both"] and self.pubmed_tools:
            pubmed_results = self.search_pubmed_with_islamic_context(query, self.max_results)
            results += pubmed_results + "\n"
        
        if search_type in ["technology", "both"] and self.arxiv_tools:
            arxiv_results = self.search_arxiv_with_islamic_context(query, self.max_results)
    ...
    def detect_query_type(self, query: str) -> str:
        ...
        is_medical = any(re.search(pattern, query_lower) for pattern in medical_patterns)
